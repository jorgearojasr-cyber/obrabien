...
import { SPECIALTIES, REGIONS, type Master } from "@/lib/data";
      ...
      <Link href={`/maestro/${m.id}`} className="card hoverable col" style={{ textDecoration: "none", padding: 0, borderRadius: 10 }}>
        <div className="row gap-12" style={{ padding: 16, borderBottom: "1px solid var(--line)" }}>
          ...
          <div className="col gap-4" style={{ flex: 1, minWidth: 0, paddingRight: 36 }}>
            ...
            <span style={{ fontSize: 13, color: "var(--mute)" }}>📍 {m.city}{m.sector && m.sector !== m.city ? ` · ${m.sector}` : ""}</span>
          </div>
        </div>
...
export default function BuscarContent({ allMaestros, realIds }: { allMaestros: Master[]; realIds: Set<string> }) {
  ...
  const results = useMemo(() => {
    return allMaestros.filter(m => {
      if (specialty && !m.specialties.some(s => s.toLowerCase().includes(specialty.toLowerCase()))) return false;
      if (city && m.city !== city && !m.sector.includes(city)) return false;
      if (!city && region) {
      ...
      if (q) {
        ...
        const haystack = `${m.name} ${m.specialties.join(" ")} ${m.city} ${m.sector} ${m.description}`.toLowerCase();
        if (!haystack.includes(needle)) return false;
      }
      return true;
    });
  }, [specialty, region, city, q, allMaestros]);
    ...
    <div style={{ background: "var(--bg)", minHeight: "60vh" }}>
      ...
      <div className="wrap" style={{ paddingTop: 28, paddingBottom: 64 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid var(--line)", padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end" }}>
            ...
            <div className="field" style={{ flex: "1 1 180px", opacity: region ? 1 : 0.45, transition: "opacity .2s" }}>
              ...
              <select className="ob-select" value={city} onChange={e => setCity(e.target.value)} disabled={!region}>
        ...
        <div style={{ fontSize: 13, color: "var(--mute)", marginBottom: 16, fontFamily: "var(--font-jetbrains), monospace" }}>
          {results.length === allMaestros.length
            ? `${results.length} maestros registrados`
            : `${results.length} resultado${results.length !== 1 ? "s" : ""} de ${allMaestros.length}`}
        </div>
        ...
        {results.length > 0 ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
            {results.map((m, i) => (
              <MasterCard key={m.id} m={m} idx={i} isFav={favs.has(m.id)} onToggleFav={() => toggle(m.id)} isReal={realIds.has(m.id)} />
            ))}
          </div>
        ) : (
