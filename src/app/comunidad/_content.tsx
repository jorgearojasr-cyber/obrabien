...
/* ── Post card ──────────────────────────────────────────────────────────── */
function PostCard({ post, pinned = false, onImageClick }: { post: ForumPost; pinned?: boolean; onImageClick: (src: string) => void }) {
  ...
  const replyCount = post.replyCount ?? post.seedReplies.length;
    ...
    <div className="post-card" style={{ borderTop: pinned ? "3px solid var(--orange)" : undefined }}>
        ...
        <div className="post-card-body">
          ...
          {post.foto_url && (
            ...
            <img
              ...
              onClick={e => { e.preventDefault(); e.stopPropagation(); onImageClick(post.foto_url!); }}
              style={{ width: "100%", maxHeight: 280, objectFit: "cover", marginTop: 12, border: "1px solid var(--line)", display: "block", cursor: "zoom-in" }}
            />
          )}
        </div>
      </Link>
      ...
      <div className="post-card-footer">
        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: 1 }}>
          <div style={{
            width: 30, height: 30, background: avatarBg, color: "#fff", flexShrink: 0,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <button
            onClick={e => { e.preventDefault(); if (voted) { setUsefulCount(c => c - 1); setVoted(false); } else { setUsefulCount(c => c + 1); setVoted(true); } }}
            style={{
              ...
              fontSize: 12, fontWeight: voted ? 700 : 500, cursor: "pointer", transition: "all .15s",
            }}>
            👍 {usefulCount}
          </button>
          ...
          <Link href={`/comunidad/${post.id}#responder`}
            style={{
              ...
              background: "#fff", color: "var(--mute)", fontSize: 12, fontWeight: 500,
            }}>
            <MessageIcon /> {replyCount}
          </Link>
...
/* ── Sort type ──────────────────────────────────────────────────────────── */
type SortMode = "recientes" | "relevantes" | "comentados";
...
export default function ComunidadContent({ dbPosts }: { dbPosts: ForumPost[] }) {
  ...
  const sorted = useMemo(() => {
    ...
    if (sort === "relevantes") {
      posts.sort((a, b) => b.useful - a.useful);
  ...
  return (
        ...
        <div className="comunidad-grid">
          ...
          <div className="col gap-0" style={{ minWidth: 0, width: "100%" }}>
            ...
            <div className="sort-toolbar">
              <span style={{ fontSize: 11.5, color: "var(--mute)", marginRight: 4, fontFamily: "var(--font-jetbrains), monospace" }}>Ordenar:</span>
              {SORT_OPTIONS.map(s => (
                <button
                  key={s.id}
                  ...
                  style={{
                    padding: "4px 12px",
                    border: `1px solid ${sort === s.id ? "var(--orange)" : "var(--line)"}`,
                    background: sort === s.id ? "rgba(232,108,28,0.08)" : "#fff",
                    color: sort === s.id ? "var(--orange)" : "var(--mute)",
                    fontSize: 12, fontWeight: sort === s.id ? 700 : 500, cursor: "pointer",
                    transition: "all .15s",
                  }}>
                  {s.label}
                </button>
          ...
          <div className="col gap-20 comunidad-sidebar">
            ...
            <div style={{ background: "#fff", border: "1px solid var(--line)", padding: 20 }}>
              ...
              <div className="col gap-10">
                {[
                  { name: "Roberto M.", role: "Gasfiter",      replies: 47, initials: "RM" },
                  { name: "Juan P.",    role: "Albañil",        replies: 38, initials: "JP" },
                  { name: "Pedro R.",   role: "Pintor",         replies: 31, initials: "PR" },
                  { name: "Carlos V.", role: "Electricista",    replies: 26, initials: "CV" },
                ].map((u, i) => (
                  <div key={u.name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ width: 32, height: 32, background: "#E86C1C", color: "#fff", display: "grid", placeItems: "center", fontFamily: "Archivo, sans-serif", fontWeight: 800, fontSize: 11, flexShrink: 0 }}>
                      {u.initials}
                    </div>
            ...
            <div style={{ background: "#fff", border: "1px solid var(--line)", padding: 20 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {["precios", "contrato", "cerámica", "gasfitería", "radier", "permiso", "pintura", "materiales", "herramientas", "técnica", "albañilería"].map(tag => (
                  <button key={tag} style={{ padding: "4px 10px", border: "1px solid var(--line)", background: "var(--bg)", color: "var(--mute)", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                    #{tag}
                  </button>
                ))}
              </div>
            ...
            {!isSignedIn && (
              <div style={{ background: "var(--navy)", color: "#fff", padding: 20 }}>
                ...
                <Link href="/registro" style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "10px", background: "var(--orange)", color: "#fff", fontWeight: 700, fontSize: 13.5, textDecoration: "none" }}>
                  Registrarme gratis
                </Link>
              </div>
            )}
    ...
    <Lightbox
      fotos={lightboxSrc ? [{ url: lightboxSrc }] : []}
      active={lightboxSrc ? 0 : null}
